...
import { logoutHandler, connectToMetamask } from '../utils/user';
...
const Navbar = () => {
    ...
    useEffect(() => {
        state?.user?.wallet_address ? (
            setUserText(state.user.wallet_address.slice(0, 4) + '...'
    ...
    const loginStyles = {
        ...
        optionMobile:
            ...
            styles.arcadeFont,
        hoverDiv: `${hoverOverConnect ? 'opacity-100' : ' opacity-0'} ${router.pathname === '/' ? 'hidden' : ''
    ...
    const getNavClasses = (menuItem) => {
        if (menuItem == activeItem) {
            return ' bg-[#00C2FF] font-bold max-w-screen w-full text-center md:w-fit md:font-normal md:bg-transparent';
        } else {
            return '';
        }
    };
	...
	@@ -234,218 +237,215 @@ const Navbar = () => {
              ...
              />
                        ...
                        {/* // padding class */}
                        <div
                            className={`${hoverOverConnect ? 'h-14 ' : 'h-0  '
                                }  opacity-0  w-full  absolute top-10  shadow-2xl`}
                        ></div>

                        <div
                            className={`${hoverOverConnect
                                ? 'opacity-100 translate-y-0 '
                                : ' opacity-0 -translate-y-16 h-0  '
                                } bg-[#0a5978]/80 transition-all  font-normal w-full overflow-hidden  absolute top-24 shadow-2xl rounded-lg  list-none  ${styles.dropdownTransitions
